...
class ShoeDetector:
    ...
    def init():
        """
        Initialize ShoeDetector building the classification model and loading its corresponding weights
        """
        print("Initializing ShoeDetector...")
        classifier = ShoeDetector.__generate_model()
        classifier.load_weights(config.CLASSIFIER_WEIGHT_PATH)
        ShoeDetector.__classification_model = classifier
        print("Done.")
    ...
    def classify_image(im):
        ...
        """
        im_copy = im.copy()
        pred = ShoeDetector.__classification_model.predict(efficientnetb7_preprocess_input(im_copy))
